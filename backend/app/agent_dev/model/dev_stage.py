...
class AgentDevStage(Base):
    ...
    stage_type: Mapped[DevStageType] = mapped_column(
        sa.String(32), comment='阶段类型(plan/design/code/review/test/deploy)'
    )
    agent_type: Mapped[DevAgentType] = mapped_column(
        sa.String(32), default=DevAgentType.CODER, comment='执行Agent类型'
    )
    ...
    status: Mapped[DevStageStatus] = mapped_column(
        sa.SmallInteger, default=DevStageStatus.PENDING, comment='状态(0等待中 1进行中 2已完成 3失败 4已跳过)'
    )

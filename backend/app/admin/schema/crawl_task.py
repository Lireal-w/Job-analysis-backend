...
class CrawlTaskSchemaBase(SchemaBase):
    ...
    schedule_type: CrawlScheduleType = Field(default=CrawlScheduleType.NONE, description='调度类型')
    ...
    batch_size: int = Field(default=100, ge=1, le=10000, description='每批处理条数')
    rate_limit: int = Field(default=0, ge=0, description='速率限制(请求/秒)')
    retry_enabled: bool = Field(default=True, description='是否启用重试')
    max_retries: int = Field(default=3, ge=0, le=20, description='最大重试次数')
    retry_delay: int = Field(default=60, ge=1, description='重试间隔(秒)')
    retry_backoff: bool = Field(default=True, description='是否启用退避策略')
    priority: CrawlPriority = Field(default=CrawlPriority.MEDIUM, description='优先级')
...
class GetCrawlTaskDetail(CrawlTaskSchemaBase):
    ...
    total_run_count: int = Field(description='总运行次数')
    total_records: int = Field(description='总采集记录数')

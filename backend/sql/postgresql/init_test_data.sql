...
-- 分配新菜单给测试角色
insert into sys_role_menu (id, role_id, menu_id)
values (5, 1, 51) on conflict (id) do nothing;
insert into sys_role_menu (id, role_id, menu_id)
values (6, 1, 52) on conflict (id) do nothing;
insert into sys_role_menu (id, role_id, menu_id)
values (7, 1, 56) on conflict (id) do nothing;
insert into sys_role_menu (id, role_id, menu_id)
values (8, 1, 60) on conflict (id) do nothing;
insert into sys_role_menu (id, role_id, menu_id)
values (9, 1, 64) on conflict (id) do nothing;
...
select setval(pg_get_serial_sequence('sys_datasource', 'id'),coalesce(max(id), 0) + 1, true) from sys_datasource;
select setval(pg_get_serial_sequence('sys_crawl_task', 'id'),coalesce(max(id), 0) + 1, true) from sys_crawl_task;
select setval(pg_get_serial_sequence('sys_quality_rule', 'id'),coalesce(max(id), 0) + 1, true) from sys_quality_rule;
select setval(pg_get_serial_sequence('sys_alert_rule', 'id'),coalesce(max(id), 0) + 1, true) from sys_alert_rule;
select setval(pg_get_serial_sequence('sys_resource_permission', 'id'),coalesce(max(id), 0) + 1, true) from sys_resource_permission;
